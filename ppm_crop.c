...
pixel **crop(char *file, metadados *img, int a, int b, int c, int d) {
  pixel **data;
  pixel **temp;
  ...
  temp = (pixel **)malloc(height * sizeof(pixel *)); //allocate some rows

  for (int i=0; i < height; i++)
    temp[i] = (pixel *)malloc(width * sizeof(pixel)); //allocate a row for each column

  for (int i=0; i < img->lin; i++){
      for (int j=0; j < img->col; j++) {
          if ((j <= c) && (j >= a) && (i <= d) && (i >= b))
            temp[i-b][j-a] = data[i][j];
      }
  }
...
int main(int argc, char *argv[]) {
  ...
  conv = crop(input, img, atoi(argv[1]), atoi(argv[2]), atoi(argv[3]), atoi(argv[4]));
