...
pixel **dflip(char *file, metadados *img) {
  pixel **data;
  pixel temp;
  data = read(file, img);

  for (int j=0; j<(img->lin); j++) {
    for(int k=0; k<(img->col)/2; k++) {
      temp = data[j][k];
      data[j][k] = data[j][(img->col)-k-1];
      data[j][(img->col)-k-1] = temp;
    }
  }

  for (int j=0; j<(img->lin)/2; j++) {
    for(int k=0; k<(img->col); k++) {
      temp = data[j][k];
      data[j][k] = data[(img->lin)-j-1][k];
      data[(img->lin)-j-1][k] = temp;
    }
  }
