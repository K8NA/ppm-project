...
pixel **rotRight(char *file, metadados *img) {
  pixel **data;
  pixel **temp;
  int aux;
  data = read(file, img);

  temp = (pixel **)malloc(img->col * sizeof(pixel *)); //allocate some rows

  for (int i=0; i < img->col; i++)
      temp[i] = (pixel *)malloc(img->lin * sizeof(pixel)); //allocate a row for each column

  for (int i=0, j=img->lin-1; i<img->lin; j--, i++) {
    for(int k=0, n=0; k<img->col; k++, n++) {
      temp[n][j] = data[i][k];
    }
  }
