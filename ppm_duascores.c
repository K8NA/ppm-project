...
pixel **duascores(char *file, metadados *img, char *fac) {
  pixel **data;
  int grey;
  data = read(file, img);

  int threshold = atoi(fac);
  ...
  for (int j=0; j<(img->lin); j++) {
    for(int k=0; k<(img->col); k++) {
      ...
      if(grey > threshold) {
        data[j][k].r = img->max; data[j][k].g = img->max; data[j][k].b = img->max;
      } else {
         data[j][k].r = 0; data[j][k].g = 0; data[j][k].b = 0;
      }
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  if (argc>2) {  //there might be an input or output file
  ...
  conv = duascores(input, img, argv[1]);
