...
pixel **addrgb(char *dr, char *dg, char *db, char *file, metadados *img) {
  pixel **data;
  data = read(file, img);
  ...
  for (int j=0; j<(img->lin); j++) {
    for(int k=0; k<(img->col); k++) {
      //compare the to check if goes beyond the limit
      if (data[j][k].r + d_r >= img->max) {
        data[j][k].r = img->max;
      } else if (data[j][k].r + d_r <= 0) {
          data[j][k].r = 0;
      } else
          data[j][k].r = data[j][k].r + d_r;

      if (data[j][k].g + d_g >= img->max) {
        data[j][k].g = img->max;
      } else if (data[j][k].g + d_g <= 0) {
          data[j][k].g = 0;
      } else
          data[j][k].g = data[j][k].g + d_g;

      if (data[j][k].b + d_b >= img->max) {
        data[j][k].b = img->max;
      } else if (data[j][k].b + d_b <= 0) {
          data[j][k].b = 0;
      } else
          data[j][k].b = data[j][k].b + d_b;
...
int main(int argc, char *argv[]) {
  ...
  conv = addrgb(argv[1], argv[2], argv[3], input, img);
