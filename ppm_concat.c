...
void concat(char *file1, char *file2, char *pfile) {
  pixel **data1;
  pixel **data2;
  pixel **printing; //the output, two images
  metadados *img1;
  metadados *img2;
  ...
  if (img1->lin < img2->lin)
    height = img1->lin;
  else
    height = img2->lin;

  pr->col = (img1->col + img2->col);
  pr->lin = height;
  pr->max = img1->max;
  strcpy(pr->magic_num, "P3");

  printing = (pixel **)malloc(height * sizeof(pixel *)); //allocate some rows

  for (int i=0; i < height; i++)
    printing[i] = (pixel *)malloc((img1->col + img2->col) * sizeof(pixel)); //allocate a row for each column

  for (int i = 0; i < height; i++){
      for (int j = 0; j < (img1->col + img2->col); j++) {
          if (j < img1->col) printing[i][j] = data1[i][j];
          else printing[i][j] = data2[i][j-img1->col];
      }
  }
...
int main(int argc, char *argv[]) {
  ...
  char *input1="0", *input2="0", *output="0";
  int f = 0;
